...
private:
   ...
   ******************************************/
  void clear(Node *node); // Recursive helper function for clear()
  Node* merge(Node* root1, Node* root2); // Recursive function to merge 2 queues given their _heap's
  void printPatientQueue(Node *node) const; // Recursive helper function for printPatientQueue()
  void dump(Node *node) const;  // Recursive helper function for dump()
  Node* copy (Node *node); // Recursive function to create deep identical copies of a queue given _heap
};
